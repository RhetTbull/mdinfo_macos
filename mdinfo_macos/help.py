...
import mdinfo_macos.mdinfo_finder as mdinfo_finder
import mdinfo_macos.mdinfo_mac as mdinfo_mac
...
def format_markdown_table(data: list[list[str] | tuple[str]], width=None) -> str:
    ...
    header = (
        ...
        + " | ".join(
            [
                str(data[0][col]).ljust(column_lengths[col])
...
def get_markdown_help() -> str:
    """Generate help text for all mdinfo plugins as markdown"""
    return "".join(
        _get_markdown_help(get_help)
        for get_help in [
            mdinfo_mac.get_template_help,
            mdinfo_finder.get_template_help,
        ]
    )


def _get_markdown_help(get_help: Callable) -> str:
    """Generate help text for this template as markdown"""
    help = ""
    for help_item in get_help():
